...
//    { label: "Jan 10", totalSales: 80, extrasSales: 10 },
//    { label: "Jan 10", totalSales: 1080, extrasSales: 120 },
...
function SalesChart({ bookings, numDays }) {
   ...
   const data = allDates.map((date) => {
      // console.log(date);
      const totalSalesOnSameDay = bookings
         .filter((booking) => isSameDay(date, new Date(booking.created_at)))
         .reduce((acc, cur) => acc + cur.totalPrice, 0);
      const extrasSalesOnSameDay = bookings
         .filter((booking) => isSameDay(date, new Date(booking.created_at)))
         .reduce((acc, cur) => acc + cur.extrasPrice, 0);
      return {
         label: format(date, "MMM dd"),
         totalSales: totalSalesOnSameDay,
         extrasSales: extrasSalesOnSameDay,
      };
   });
   // console.log(data);
   const colors =
   ...
   return (
      <StyledSalesChart>
         <Heading as="h2">
            Sales ({format(subDays(new Date(), numDays - 1), "MMM dd yyyy")} -
